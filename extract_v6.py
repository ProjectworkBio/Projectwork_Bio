...
def process_csv_file(file_path):
    ...
    for sent, pmid, doc in zip(filtered_sentences, filtered_pmids, docs):
        protein_entities = [e for e in doc.ents if e.label_ in {"GENE_OR_GENE_PRODUCT"}]
        matched_raw = {}
        sent_tokens = set(normalize(sent).split())

        for ent in protein_entities:
            vec = get_span_vector(ent)
            for idx, norm_name in enumerate(protein_names):
                prot_vec = get_protein_vector(idx)
                if cosine_safe(vec, prot_vec) >= SIM_THRESHOLD and token_overlap(protein_token_sets[norm_name], sent_tokens) >= MIN_TOKEN_OVERLAP:
                    matched_raw[norm_name] = ent.text 

        if matched_raw:
            results.append({
                "PubMedID": pmid,
                "Sentence": sent,
                "Matched_Proteins": "; ".join(sorted(matched_raw.values())),
                "UniProtIDs": "; ".join(sorted({protein_map[m] for m in matched_raw.keys()}))
            })
    ...
    if results:
        ...
        logger.info(f"Empty result in file: {file_path}")
